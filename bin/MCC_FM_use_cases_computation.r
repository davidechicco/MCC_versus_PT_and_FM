...
FowlkesMallowsIndex<-  function(thisTP, thisTN, thisFP, thisFN)
{

        thisTPR <- thisTP / (thisTP + thisFN)
        thisPPV <- thisTP / (thisTP + thisFP)

        thisFM <- sqrt(thisTPR * thisPPV)
...
cat("TP TN FP FN TPR TNR PPV NPV FM MCC normMCC diff\n")
    ...
    for(TN in seq(from = START, to = END, by = STEP)) {
        for(FP in seq(from = START, to = END, by = STEP)) {
            for(FN in seq(from = START, to = END, by = STEP)) {
                ...
                FMhere <- FowlkesMallowsIndex(TP, TN, FP, FN)
                MCChere <- mcc(TP, TN, FP, FN)
                normMCChere <- normMCC(TP, TN, FP, FN)
                
                if(is.null(MCChere) == FALSE) {
                
                    diff <- abs(FMhere - normMCChere)
